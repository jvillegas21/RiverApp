...
function estimateTimeToFloodEnhanced(currentStage, floodStages, flowTrend, precipitation, riskScore) {
  // Calculate time based on multiple factors
  const stageRatio = currentStage / floodStages.minor;
  ...
  if (currentStage >= floodStages.minor && flowTrend > 0.1) return '1-2 hours';
  if (currentStage >= floodStages.action && precipitationIntensity > 60) return '2-4 hours';
  
  // Predictive conditions
  if (stageRatio > 0.8 && flowTrend > 0.15 && precipitationIntensity > 50) return '4-8 hours';
  if (stageRatio > 0.6 && flowTrend > 0.1 && precipitationIntensity > 40) return '8-12 hours';
  if (stageRatio > 0.4 && precipitationIntensity > 70) return '12-24 hours';
  
  // No immediate threat
  if (riskScore.probability < 30) return 'No immediate threat';
  if (riskScore.probability < 50) return 'Monitor conditions';
